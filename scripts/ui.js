...
function insertSidebar() {
    ...
    expander.classList = "sticky top-0 mb-1.5 flex items-center justify-between z-10 h-14 p-2 font-semibold dark:bg-gray-800";
    ...
    refreshButton.classList = "sticky top-0 mb-1.5 flex items-center justify-between z-10 h-14 p-2 font-semibold dark:bg-gray-800";
...
function insertLinks(links) {
    let questionMark = document.querySelector(
...
let menuItemsContainer = questionMark.querySelector("[role='menu']");
    ...
    if (menuItemsContainer) {
      let linkAlreadyExists = menuItemsContainer.querySelector(
        ".chatgpt-citation-item"
      );
      if (!linkAlreadyExists) {
        let parentLinkContainer = menuItemsContainer.querySelector("div[role='menu']");
        console.log(parentLinkContainer);
        if (parentLinkContainer.length == 0) {
            console.log("No parent link container found");
            return;
          }
        links.forEach((link) => {
          ...
          let svgIcon = document.createElementNS(
          ...
          // Create the path element inside the SVG for the icon
          let svgPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
          );
          svgPath.setAttribute("d", link.svg);
          ...
          // Append SVG icon and text to the link
          customLink.appendChild(svgIcon);
          customLink.appendChild(spanText);
  
          // Append the link to the parent container
          menuItemsContainer.append(customLink);
        });
      }
    }
  }
